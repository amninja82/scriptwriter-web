...
"""
    ...
    def get_input_safely(self, prompt, is_password=False):
        """安全获取用户输入（兼容 Windows）"""
        try:
            if is_password:
                # 尝试使用 getpass，如果失败则使用普通 input
                try:
                    import getpass
                    return getpass.getpass(prompt)
                except (ImportError, Exception):
                    # getpass 失败，使用普通 input
                    print("(提示: 输入时会显示在屏幕上，这是正常的)")
                    return input(prompt)
            else:
                return input(prompt)
    ...
    def connect(self):
        ...
        print("(注意：Token 输入时会显示在屏幕上，请注意隐私)\n")

        # 使用安全的输入方式
        self.api_token = self.get_input_safely("API Token: ", is_password=True)
...
"""
    ...
    def run(self):
        ...
        while True:
            try:
                # 获取用户输入
                user_input = self.get_input_safely("\n💬 请输入消息 (输入 'quit' 退出): ")
